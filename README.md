...
**問題定義 → 生成資料 → 資料探索(EDA) → 資料前處理 → 模型訓練 → 預測 → 比較 → 錯誤分析 → 特徵工程 → 模型調參 → 集成學習 → 最後做成使用者可操作介面 → 上傳到 GitHub → 使用 Streamlit Community Cloud 部署**
...
在 V1 版本中，Sine 與 Cosine 是較明顯的混淆來源，因此進一步針對這兩類函數的錯誤樣本進行分析。相關分析顯示，在特定測試子集與參數條件下，兩者容易產生分類混淆。

而在 V2 版本中，Logarithmic 與 Reciprocal 也是較容易混淆的類別之一，部分錯誤樣本的比例約達 30%。
### 6. 特徵工程
...
### 結論
> 當原始資料資訊較充足時，模型可以直接從取樣值中學習函數形狀；而當資料量減少時，適當加入具有數學意義的特徵，可以補充原始取樣值不足的資訊。
