...
x = np.linspace(-2, 2, 100)         # x 座標

def generate_function(label, x, rng):       # 函數產生器
    param_1 = np.nan
    param_2 = np.nan
    param_3 = np.nan
    param_4 = np.nan
    ...
    elif label == "exponential":            # Exponential: y = a * exp(bx) + c
    ...
    elif label == "logarithmic":            # Logarithmic: y = a * log(|x| + 1) + bx + c
        a = rng.uniform(-3, 3)
        b = rng.uniform(-1, 1)
        c = rng.uniform(-2, 2)
        param_1 = a
    ...
    # noise_sigma
    y_std = np.std(y_true)
    noise_ratio = rng.uniform(0.02, 0.08)
...
# 儲存檔案
dataset_df.to_csv(r'D:\Python\我的AI作品集\專案1_數學函數辨識\data\v1\function_dataset.csv', index=False)
metadata_df.to_csv(r'D:\Python\我的AI作品集\專案1_數學函數辨識\data\v1\function_metadata.csv', index=False)
print("數據資料建立完成！")