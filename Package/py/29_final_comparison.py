...
# 載入 V1 結合後的資料
data1 = pd.read_csv(f'{DATA_DIR}/v1/function_dataset_reset.csv')
...
# 載入 V1 最終模型
model_v1_package = joblib.load(f'{MODEL_DIR}/v1/final_model.joblib')
v1_final_model = model_v1_package['model']
v1_final_model.fit(X1_train, y1_train)
...
# 載入 V2 結合後的資料
data2 = pd.read_csv(f'{DATA_DIR}/v2/function_dataset_reset_v2.csv')
...
# 載入 V2 最終模型
model_v2_package = joblib.load(f'{MODEL_DIR}/v2/final_model_v2.joblib')
v2_final_model = model_v2_package['model']
v2_final_model.fit(X2_train, y2_train)
...
comparison_df = pd.DataFrame([{"version": "V1",
                              ...
                              {"version": "V2",
                               ...
                               "error_count": v2_error,
                               "training_time_seconds": v2_training_time}])
comparison_df.to_csv(f'{DATA_DIR}/final_results.csv', index=False)

comparison_df.to_html(f'{DATA_DIR}/final_results.html')
...
with open(f'{DATA_DIR}/final_comparison_results.json', 'w', encoding='utf-8') as f:
